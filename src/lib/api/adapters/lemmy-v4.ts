...
function convertCommunity(
  ...
  return {
    ...
    title: (community as any).title || community.name, // Lemmy v1.0.0 has title field
...
export class LemmyV4Api implements ApiBlueprint<lemmyV4.LemmyHttp> {
  ...
  async getPosts(form: Forms.GetPosts, options: RequestOptions) {
    ...
    // Lemmy v1.0.0 returns { posts: [], next_page: ... } not { items: [] }
    const postItems = (posts as any).posts || (posts as any).items || [];
